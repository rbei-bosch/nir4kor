...
OTELCOLEXT_TAG="$(cat $ROOT_DIRECTORY/prerequisite_settings.json | jq .otelagent.version | tr -d '"')"
OETLCOLEXP_TAG="$(cat $ROOT_DIRECTORY/prerequisite_settings.json | jq .otelexporter.version | tr -d '"')"
...
if grep -q ghcr.io $HOME/.docker/config.json; then
    ...
    docker pull ghcr.io/softwaredefinedvehicle/sdv-edge-otel/otelcol-sdv-ext:$OTELCOLEXT_TAG
    docker tag ghcr.io/softwaredefinedvehicle/sdv-edge-otel/otelcol-sdv-ext:$OTELCOLEXT_TAG localhost:12345/otelcol-sdv-ext:$OTELCOLEXT_TAG
    docker push localhost:12345/otelcol-sdv-ext:$OTELCOLEXT_TAG

    docker pull ghcr.io/softwaredefinedvehicle/sdv-edge-otel/otelcol-sdv-exporter:$OETLCOLEXP_TAG
    docker tag ghcr.io/softwaredefinedvehicle/sdv-edge-otel/otelcol-sdv-exporter:$OETLCOLEXP_TAG localhost:12345/otelcol-sdv-exporter:$OETLCOLEXP_TAG
    docker push localhost:12345/otelcol-sdv-exporter:$OETLCOLEXP_TAG
